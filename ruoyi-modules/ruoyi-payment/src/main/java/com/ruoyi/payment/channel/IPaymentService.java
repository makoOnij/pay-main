...
public interface IPaymentService {
    ...
    String preCheck(UnifiedOrderRQ bizRQ, PayOrderDto payOrderDto);

    /** 自定义支付订单号， 若返回空则使用系统生成订单号 */
    String customPayOrderId(UnifiedOrderRQ bizRQ, PayOrderDto payOrderDto);


    /** 调起支付接口，并响应数据；  内部处理普通商户和服务商模式  **/
    AbstractRS pay(UnifiedOrderRQ bizRQ, PayOrderDto payOrderDto, TenantDto tenantDto);
