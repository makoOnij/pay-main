 ...
 */
@Slf4j
public class DuplicateKeyAddMethod extends AbstractMethod {
    public DuplicateKeyAddMethod() {
        super("duplicateKeyAdd");
    }
    ...
    private final Predicate<TableFieldInfo> predicate = p -> !Objects.equals(p.getProperty(), "tenantName")
            && !Objects.equals(p.getProperty(), "tenantId")
            && !Objects.equals(p.getProperty(), "createTime");
    /**
     * sqlDuplicate
     *
     * @return
    ...
    private String sqlDuplicate(TableInfo tableInfo) {
        ...
        List<TableFieldInfo> fieldList = tableInfo.getFieldList().stream().filter(predicate).collect(Collectors.toList());
